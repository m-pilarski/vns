...
#' @param .install_miniconda DESCRIPTION.
...
setup_vns_condaenv <- function(
  .conda_envname="vns_condaenv", .install_miniconda=NULL, .create_condaenv=NULL
){
  checkmate::assert(
    ...
    checkmate::check_logical(
      .install_miniconda, len=1, null.ok=TRUE, any.missing=FALSE
    ),
    checkmate::check_logical(
  ...
  if(is(try(reticulate::conda_version()), "try-error")){
    ...
    if(.install_miniconda){
      ...
      rlang::abort(c(
        ...
        i="Please rerun with .install_miniconda=TRUE or install conda manually"
      ))
    }
  }
